...
var WampCall = function(client, callId) {
	this.client = client;
	this.callId = callId;
	var self = this;
	this.cb = function cb(err, result) {
		var msg;
		if (err) {
		  msg = [protocol.TYPE_ID_CALL_ERROR, self.callId, 'http://autobahn.tavendo.de/error#generic', err.toString()];
		} else {
		  msg = [protocol.TYPE_ID_CALL_RESULT, self.callId, result];
		}
		self.client.send(JSON.stringify(msg));
	};
};
...
handlers[protocol.TYPE_ID_CALL] = function(client, args) {
  ...
  args = args || [];
  var wampcall = new WampCall(client, callId);
  this.emit('call', procUri, args, wampcall.cb);
};
