...
handlers[protocol.TYPE_ID_CALL] = function(client, args) {
  ...
  cb = (function(callId) {
    return function(err, result) {
      var msg;
      if (err) {
        msg = [protocol.TYPE_ID_CALL_ERROR, callId, (err.uri || protocol.URI_WAMP_ERROR_GENERIC), err.toString()];
      } else {
  ...
  // handle auth and authreq rpc calls
  if (auth.isAuthUri(procUri)) {
    auth.handleAuthRpc.call(this, client, procUri, args, cb);
    return;
  }
