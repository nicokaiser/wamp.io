...
Session.prototype.publish = function (topicUri, data, excludeMeOrExclude, exclude) {
    var ev = [protocol.TYPE_ID_PUBLISH, topicUri, event, exclude];

    if (excludeMeOrExclude)
        ev.push(excludeMeOrExclude);

    if (exclude)
        ev.push(exclude);

    this._send(JSON.stringify(ev));
};

Session.prototype.subscribe = function (topicUri, callback) {
    this._send(JSON.stringify([protocol.TYPE_ID_SUBSCRIBE, topicUri]));
    this._subscriptions[topicUri] = callback;
};

Session.prototype.unsubscribe = function (topicUri) {

    this._send(JSON.stringify([protocol.TYPE_ID_UNSUBSCRIBE, topicUri]));
